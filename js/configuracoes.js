...
Huddle.Configuracoes = {
    ...
    Huddle.Utils.$("app").innerHTML = `
      <div class="tela tela-configuracoes">
        ...
        <div class="grid-cards grid-configuracoes">
          ...
          <button class="card card-configuracao" onclick="Huddle.Configuracoes.renderMetas()">
            <span class="icone-config-card">🎯</span>
            <strong>Metas</strong>
            <small>Conformidade: ${Number(metaConformidade).toFixed(1).replace(".", ",")}%</small>
          </button>

          <button class="card card-configuracao" onclick="Huddle.Configuracoes.renderBackup()">
            <span class="icone-config-card">💾</span>
            <strong>Backup local</strong>
            <small>Exportar/importar</small>
          </button>
        </div>
  ...
  async renderMetas() {
    ...
    Huddle.Utils.$("app").innerHTML = `
      <div class="tela">
        ...
        <form class="card form-grid" onsubmit="Huddle.Configuracoes.salvarMetas(event)">
          <div class="form-linha">
            <label for="config_meta_conformidade">Meta de conformidade (%)</label>
            <input
              id="config_meta_conformidade"
              type="number"
              min="0"
              max="100"
              step="0.1"
              required
              value="${Huddle.Utils.escapeHtml(metaConformidade)}"
            >
            <small class="texto-apoio sem-margem">
              Padrão recomendado: 80%.
            </small>
          </div>

          <div class="acoes">
            <button type="button" class="btn-secundario" onclick="Huddle.Configuracoes.renderHome()">
              Cancelar
            </button>
            <button type="submit" class="btn-principal">
              Salvar metas
  ...
  async salvarMetas(event) {
    event.preventDefault();

    const campo = Huddle.Utils.$("config_meta_conformidade");
    const valor = Number(campo.value);

    if (!Number.isFinite(valor) || valor < 0 || valor > 100) {
      Huddle.Utils.toast("Informe uma meta entre 0 e 100%.");
      return;
    }

    const existente = await Huddle.DB.get("meta", "config_meta_conformidade");
    const agora = Huddle.Utils.agoraISO();
    ...
    await Huddle.DB.addLog({
      tipo: "configuracao",
      acao: "Meta de conformidade alterada",
      detalhe: `${valor.toFixed(1).replace(".", ",")}%`,
      usuario: "Configuração"
    });

    Huddle.Utils.toast("Meta salva.");
  ...
  async exportarBackup() {
    ...
    link.href = url;
    link.download = `huddleflow_backup_${data}_${hora}.json`;
